...
|#  |태스크                               |상태  |우선순위|비고                                         |
|---|----------------------------------|----|----|---------------------------------------------|
|2.1|ShyftClient 래퍼 구현 (REST API)      |✅ 완료|🔴 높음|2024-12-02 완료, src/services/external/ShyftClient.ts|
|2.2|Yellowstone gRPC 클라이언트 초기화        |✅ 완료|🔴 높음|2024-12-02 완료, src/services/external/GrpcClient.ts |
|2.3|연결 재시도 로직 구현 (exponential backoff)|✅ 완료|🟡 중간|2024-12-02 완료, 모든 클라이언트에 포함                    |
...
|#  |태스크                         |상태  |우선순위|비고                                            |
|---|----------------------------|----|----|----------------------------------------------|
|3.1|트랜잭션 스트리머 구현 (스왑 이벤트)       |✅ 완료|🔴 높음|2024-12-02 완료, TransactionStreamer.ts           |
|3.2|Pump.fun 신규 토큰 감지           |✅ 완료|🔴 높음|2024-12-02 완료, NewTokenDetector.ts              |
|3.3|LetsBONK.fun 신규 토큰 감지       |✅ 완료|🟡 중간|2024-12-02 완료, Moonshot 포함                     |
|3.4|Pool Account 변경 구독 (유동성 실시간)|✅ 완료|🔴 높음|2024-12-02 완료, PoolMonitor.ts                   |

**Phase 3 완료 조건**: 실시간 토큰 생성/거래 이벤트 수신 확인 ✅ (API 키 필요)
...
|#  |태스크                       |상태  |우선순위|비고                                              |
|---|--------------------------|----|----|------------------------------------------------|
|4.1|Jupiter Price API v3 클라이언트|✅ 완료|🔴 높음|Phase 2에서 완료, JupiterPriceClient.ts               |
|4.2|온체인 가격 계산 엔진 (Fallback)   |✅ 완료|🟡 중간|2024-12-02 완료, OnchainPriceCalculator.ts          |
|4.3|SOL/USD Oracle 연동         |✅ 완료|🔴 높음|2024-12-02 완료, SolUsdOracle.ts (Pyth Network)     |
|4.4|가격 캐시 (Redis, TTL 30초)    |✅ 완료|🟡 중간|2024-12-02 완료, PriceService.ts                    |
...
|#  |태스크                      |상태  |우선순위|비고                               |
|---|-------------------------|----|----|----------------------------------|
|6.1|전체 홀더 스캐너 (SDK getOwners)|✅ 완료|🔴 높음|2024-12-02 완료, HolderScanner.ts     |
|6.2|Top 10 비율 계산             |✅ 완료|🟡 중간|2024-12-02 완료, Gini 계수 포함          |
|6.3|홀더 스냅샷 저장                |✅ 완료|🟡 중간|2024-12-02 완료, HolderSnapshot.ts    |
...
|----------|---------|---|------|
|M1: 인프라 구축|Phase 1-2|-  |✅ 완료  |
|M2: 실시간 수집|Phase 3-4|-  |🔄 진행 중|
